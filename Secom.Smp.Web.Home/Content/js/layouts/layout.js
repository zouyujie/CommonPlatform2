...
var Layout = function () {
    ...
    var Cookies = false;
    var layoutImgPath = 'content/img/';

    var layoutCssPath = 'content/css/';
    ...
    var handleSidebarAndContentHeight = function () {
        ...
        var body = $('body');
        var height;
        var indexNav = $("#indexNav").css("display") == "none" ? 0 : $("#indexNav").outerHeight();
        var indexNamv = (indexNav == undefined || indexNav == null) ? 0 : indexNav;

        if (body.hasClass("page-footer-fixed") === true && body.hasClass("page-sidebar-fixed") === false) {
            var available_height = App.getViewPort().height - $('.page-footer').outerHeight() - $('.page-header').outerHeight()-indexNamv;
            if (content.height() < available_height) {
                content.css('min-height', available_height);
            }
        } else {
            if (body.hasClass('page-sidebar-fixed')) {
                height = _calculateFixedSidebarViewportHeight() -indexNamv;
                if (body.hasClass('page-footer-fixed') === false) {
                    height = height - $('.page-footer').outerHeight();
                }
            } else {
                var headerHeight = $('.page-header').outerHeight() +indexNamv;
    ...
    var handleSidebarMenu = function () {
        ...
        $('.page-sidebar').on('click', ' li > a.ajaxify', function (e) {
            ...
            Layout.loadAjaxContent(url,undefined,undefined, $(this));
        });
    ...
    return {
        ...
        //界面地址，[容器ID]，[是否显示加载中图片],[需要定位到的菜单模块]，[顶部菜单索引序号，从1开始]
        loadAjaxContent: function (url, divId, notShowLoading, sidebarMenuLink, index) {
            var divId = divId == undefined ? "page-content" : divId;
            var pageContent = $("#" + divId);// $('.page-content .page-content-body');
            ...
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (sidebarMenuLink!=undefined&& sidebarMenuLink.size() > 0 && sidebarMenuLink.parents('li.open').size() === 0) {
                    ...
                    //    //第一项点击
                    //    $("#leftMenu .ajaxify").first().click();
                    //}
                },
                error: function (res, ajaxOptions, thrownError) {
                    App.stopPageLoading();
                    //pageContent.html('<h4>Could not load the requested content.</h4>');
                    pageContent.load("/404.html");
