...
* 文件名: CompositeSearchingController.cs
* 创建者: 邹琼俊
* 创建时间: 08/10/2017 16:23:54
* 版权所有： 紫衡技术
******************************************************************/
...
{
    public class CompositeSearchingController : BaseController
    {
        MaintainBaseServices baseServices = new MaintainBaseServices();
        CompositeSearchingServices _CompositeSearchingServices = new CompositeSearchingServices();
        ...
        public JsonResult List(MaintainStandardFilters filter)
        {
            ...
            var dataSource = _CompositeSearchingServices.DataSource(); 
        ...
        {
            CreateSubPageHead("工单详情");
            var model = _CompositeSearchingServices.DataSource().Find(x => x.ID == Id);
            return View(model);
        }
    }
}